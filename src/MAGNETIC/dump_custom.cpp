...
int DumpCustom::count()
{
  ...
  if (nthresh) {
    ...
    for (int ithresh = 0; ithresh < nthresh; ithresh++) {
      ...
      } else if (thresh_array[ithresh] == MAGFX) {
        ptr = &atom->mag_f[0][0];
        nstride = 3;
      } else if (thresh_array[ithresh] == MAGFY) {
        ptr = &atom->mag_f[0][1];
        nstride = 3;
      } else if (thresh_array[ithresh] == MAGFZ) {
        ptr = &atom->mag_f[0][2];
        nstride = 3;
...
int DumpCustom::parse_fields(int narg, char **arg)
{
  ...
  for (int iarg = 5; iarg < narg; iarg++) {
    ...
    if (strcmp(arg[iarg],"id") == 0) {
      ...
      pack_choice[i] = &DumpCustom::pack_mag_fx;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"magfy") == 0) {
      pack_choice[i] = &DumpCustom::pack_mag_fy;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"magfz") == 0) {
      pack_choice[i] = &DumpCustom::pack_mag_fz;
      vtype[i] = DOUBLE;
...
int DumpCustom::modify_param(int narg, char **arg)
{
  ...
  if (strcmp(arg[0],"thresh") == 0) {
    ...
    else if (strcmp(arg[1],"magfx") == 0) thresh_array[nthresh] = MAGFX;
    else if (strcmp(arg[1],"magfy") == 0) thresh_array[nthresh] = MAGFY;
    else if (strcmp(arg[1],"magfz") == 0) thresh_array[nthresh] = MAGFZ;
...
void DumpCustom::pack_mag_fx(int n)
{
  double **mag_f = atom->mag_f;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = mag_f[clist[i]][0];
    n += size_one;
  }
}
...
void DumpCustom::pack_mag_fy(int n)
{
  double **mag_f = atom->mag_f;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = mag_f[clist[i]][1];
    n += size_one;
  }
}
...
void DumpCustom::pack_mag_fz(int n)
{
  double **mag_f = atom->mag_f;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = mag_f[clist[i]][2];
    n += size_one;
  }
}
